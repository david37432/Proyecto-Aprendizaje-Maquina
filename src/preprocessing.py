...
def run_feature_engineering(
    ...
    output_dir: str = '../data/prepared/datos_df.pkl'
) -> None:
    """
    Realiza la ingeniería de características sobre el dataset de productividad estudiantil.

    Parámetros
    ----------
    data_path   : ruta al archivo CSV de entrada.
    output_path : ruta donde se guardará el DataFrame preparado (formato pickle).
    """
    ...
    # Crear la carpeta ..\data\prepared si no existe
    DATA_PREPARED_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', 'data', 'prepared')
    os.makedirs(DATA_PREPARED_PATH, exist_ok=True)
    df.to_pickle(os.path.join(DATA_PREPARED_PATH, 'datos_df.pkl'))  # Guardar el DataFrame preparado para su uso en entrenamiento e inferencia
    print(f"✅ Dataset preparado y guardado en '{os.path.join(DATA_PREPARED_PATH, 'datos_df.pkl')}' con {len(df):,} filas y {len(df.columns)} columnas de features.")
